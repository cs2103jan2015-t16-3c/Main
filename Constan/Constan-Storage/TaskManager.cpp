...
void TaskManager::addTask(string taskName, string startDate, string startTime, string endDate, string endTime) {
	string type = getType(taskName, startDate, startTime, endDate, endTime);
...
vector<Task>* TaskManager::retrieveTimedTask(string timeIndicator) {
	...
	if (timeIndicator != "all") {
		...
		for (int i = 0; i < timedTaskVector.size(); i++) {
			date = timedTaskVector.at(i).getStartDate();
			if (date == timeIndicator) {
